...
class FinTA:
    ...
    @classmethod
    def Relative_EMA(cls, ohlc: DataFrame, period1: int = 20, period2: int = 50, column: str = "close",
    ...
    @classmethod
    def Log_Pct_change(cls, ohlc: DataFrame, period: int = 20, column: str = "close") -> Series:
        "Percent of change in a specific period"
        return pd.Series(
            np.round(ohlc[column].pct_change(periods=period) * 100, 1), name=f"Pct_change_{period}")
    ...
    def Max_increase_pct(cls, ohlc: DataFrame, period: int = 20, column: str = "close") -> Series:
        ...
        for i in range(1, len(ohlc)):
            ...
            min_price_in_the_last_n_values = (ohlc['low'].iloc[max(0, i - period):i]).min()
        ...
        return pd.Series(
            np.round(results, 2), name=f"Max_increase_pct_{period}", index=ohlc.index)
    ...
    def Max_decrease_pct(cls, ohlc: DataFrame, period: int = 20, column: str = "close") -> Series:
        ...
        for i in range(1, len(ohlc)):
            curr_price = ohlc[column].iloc[i]
            max_price_in_the_last_n_values = (ohlc['high'].iloc[max(0, i - period):i]).max()
        ...
        return pd.Series(
            np.round(results, 2), name=f"Max_decrease_pct_{period}", index=ohlc.index)
    ...
    # ---------------------------------------------------------------------
    @classmethod
    def SMA(cls, ohlc: DataFrame, period: int = 20, column: str = "close") -> Series:
        """
    ...
    def PPO(
        ...
        PPO_signal = pd.Series(
            PPO.ewm(ignore_na=False, span=signal, adjust=adjust).mean(),
            name=f"SIGNAL_{period_fast}_{period_slow}_{signal}"
        )
    ...
    def VW_MACD(
        ...
        MACD_signal = pd.Series(
            MACD.ewm(ignore_na=False, span=signal, adjust=adjust).mean(), name="VW_MACD_Signal"
        )
    ...
    def EV_MACD(
        ...
        MACD_signal = pd.Series(
            MACD.ewm(ignore_na=False, span=signal, adjust=adjust).mean(), name="EV_MACD_Signal"
        )
    ...
    def MOBO(
        ...
        BB = FinTA.BBANDS(ohlc, period=10, std_multiplier=0.8, column=column)
        return BB
    ...
    def BBWIDTH(
        ...
        BB = FinTA.BBANDS(ohlc, period, MA, column)
    ...
    def DMI(cls, ohlc: DataFrame, period: int = 14, adjust: bool = True) -> DataFrame:
        ...
        diplus = pd.Series(
            ...
            * (ohlc["plus"] / cls.ATR(ohlc, period))
            ...
            .mean(),
            name="{0}period DI+".format(period),
        )
        diminus = pd.Series(
            100
    ...
    def ADX(cls, ohlc: DataFrame, period: int = 14, adjust: bool = True) -> Series:
        ...
        dmi_plus = dmi["{0}period DI+".format(period)]
        dmi_minus = dmi["{0}period DI-".format(period)]
        return pd.Series(
            100
            * (abs(dmi_plus - dmi_minus) / (dmi_plus + dmi_minus))
            .ewm(alpha=1 / period, adjust=adjust)
            .mean(),
    ...
    @classmethod
    def UO(cls, ohlc: DataFrame, period1: int = 7, period2: int = 14, period3: int = 28, column: str = "close") -> Series:
        """
        *********** Edited by Mohammad ****************
        ...
        Average7 = bp.rolling(window=period1).sum() / cls.TR(ohlc).rolling(window=7).sum()
        Average14 = bp.rolling(window=period2).sum() / cls.TR(ohlc).rolling(window=14).sum()
        Average28 = bp.rolling(window=period3).sum() / cls.TR(ohlc).rolling(window=28).sum()

        return pd.Series(
            (100 * ((4 * Average7) + (2 * Average14) + Average28)) / (4 + 2 + 1), name=f"UO_{column}"
        )
    ...
    @classmethod
    def MI(cls, ohlc: DataFrame, period: int = 9, window: int = 25, adjust: bool = True) -> Series:
        """
                *********** Edited by Mohammad ****************
        ...
        return pd.Series(mass.rolling(window=window).sum(), name="Mass Index")
    ...
    def KST(
        ...
        signal = pd.Series(k.rolling(window=10).mean(), name="KST_Signal")
    ...
    @classmethod
    @inputvalidator(input_="ohlcv")
    def OBV_Normalized(cls, ohlcv: DataFrame, column: str = "close") -> Series:
        """
        source: https://en.wikipedia.org/wiki/On-balance_volume#The_formula
        https://stackoverflow.com/questions/52671594/calculating-stockss-on-balance-volume-obv-in-python
        """
        obv = (np.sign(ohlcv[column].diff()) * ohlcv['volume']).fillna(0).cumsum()
        obv_series = pd.Series(obv/ohlcv['volume'], name="OBV_Normalized")
        obv_series.replace([np.inf, -np.inf], np.nan, inplace=True)
    ...
    def EBBP(cls, ohlc: DataFrame) -> DataFrame:
        ...
        bull_power = pd.Series(ohlc["high"] - cls.EMA(ohlc, 13), name="EBBP_Bull")
        bear_power = pd.Series(ohlc["low"] - cls.EMA(ohlc, 13), name="EBBP_Bear")
    ...
    @classmethod
    def COPP(cls, ohlc: DataFrame, period1:int=14, period2:int=11, adjust: bool = True) -> Series:
        """
        ************* EDITED BY MOHAMMAD
        ...
        roc1 = cls.ROC(ohlc, period1)
        roc2 = cls.ROC(ohlc, period2)
    ...
    def BASP(cls, ohlc: DataFrame, period: int = 40, adjust: bool = True) -> DataFrame:
        ...
        nbfraw = pd.Series(nbp * nv, name=f"BASP_Buy_{period}")   # modified
        nsfraw = pd.Series(nsp * nv, name=f"BASP_Sell_{period}")  # modified
    ...
    def BASPN(cls, ohlc: DataFrame, period: int = 40, adjust: bool = True) -> DataFrame:
        ...
        nbf = pd.Series((nbp * nv).ewm(span=20, adjust=adjust).mean(), name="BASPN_Buy")
        nsf = pd.Series((nsp * nv).ewm(span=20, adjust=adjust).mean(), name="BASPN_Sell")
    ...
    @classmethod
    @inputvalidator(input_="ohlcv")
    def VPT_Moh(cls, ohlc: DataFrame) -> Series:
        """
        Volume Price Trend
        The Volume Price Trend uses the difference of price and previous price with volume and feedback to arrive at its final form.
        If there appears to be a bullish divergence of price and the VPT (upward slope of the VPT and downward slope of the price) a buy opportunity exists.
    ...
    def FVE(cls, ohlc: DataFrame, period: int = 22, factor: int = 0.3) -> Series:
        ...
        return pd.Series((_sum / smav) / period * 100, name=f"FVE_{period}")
    ...
    def VFI(
        ...
        vfi = pd.Series(
            ...
            name="{0}_{1}_VFI".format(period, smoothing_factor),
        )
